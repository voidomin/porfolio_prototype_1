...
- [🚩] `100vh`/mobile-viewport bug — re-examined both halves of this. `ProjectsSection.tsx`'s `window.innerHeight`-based pinned-scroll math turns out not to be reachable by the classic mobile address-bar-resize case at all: `DesktopLayout` (the only place that math runs) is gated behind `useIsDesktopPointer`'s `(pointer: fine)` check, which by definition excludes every touch-primary device — the only devices whose browser chrome dynamically resizes like this. No code change made there; closing that half of the concern rather than leaving it open on a stale assumption. `HeroSection.tsx`'s `min-h-screen` → `min-h-dvh` fix (its bottom-anchored "Begin the journey" scroll cue could render below the fold while the address bar is still showing) is a real, standard fix for a real, reachable bug, but Playwright's emulated mobile viewports have no live toolbar to shrink/grow — `vh` and `dvh` compute identically in headless emulation, so this specific fix's actual payoff still needs a real phone to confirm, not just a clean Playwright run.
