...
- [ ] `/images/blog/protein-research` (a `next/og` `ImageResponse` edge route, used as the homepage's blog-preview thumbnail) reliably 404s specifically in a real production build running in GitHub Actions' `ubuntu-latest` — confirmed directly, not assumed: curled the exact same route against a real `next build && next start` on this machine (Windows) and got a clean `200` with a valid PNG, while the identical request 404'd twice in a row (initial attempt + Playwright's own retry) in the same CI run. Root cause not identified from this environment — a plausible but unconfirmed theory is a `next/og`/Satori font-rendering difference on a bare Linux container (neither this route nor the sitewide `opengraph-image.tsx` explicitly provides a `fonts` option, relying entirely on whatever default Satori falls back to), but this needs someone with real CI shell access (SSH into a run, or reproduce in a matching Docker container) to confirm rather than guessing further. Currently worked around, not fixed: `e2e/responsive-a11y.spec.ts`'s console-error check explicitly filters this one known error by name rather than ignoring console errors generally.
