...
export const MountainAscentHUD = () => {
  ...
  return (
    <div className="fixed right-6 lg:right-10 top-1/2 -translate-y-1/2 z-40 hidden md:flex flex-col items-center select-none pointer-events-auto">
      {/* HUD Container */}
      <div className="relative flex flex-col items-center py-4 px-2 rounded-2xl bg-black/10 backdrop-blur-sm border border-white/5 shadow-2xl">
        
        {/* Base Hiking Trail (Background Dotted Line) */}
        <div className="absolute w-[2px] h-[calc(100%-40px)] bg-white/10 top-5 bottom-5 border-dashed border-r border-white/5" />

        {/* Lit Hiking Trail (Active Scroll Track) */}
        <div 
          className="absolute w-[2px] bg-gradient-to-b from-dawn-400 via-forest-400 to-amber-500 top-5 transition-all duration-300 origin-top"
          style={{
            height: `${(scrollPercentage / 100) * 85}%`,
            maxHeight: "100%",
          }}
        />
          ...
          {hudItems.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <AnimatePresence>
                  {isHovered && (
                    <motion.div
                      ...
                      className={cn(
                        "absolute right-full mr-2 px-3 py-1.5 rounded-lg text-xs font-semibold tracking-wider whitespace-nowrap shadow-md backdrop-blur-md border",
                        isActive
                ...
                {/* Trail Dot Checkpoint */}
                <motion.div
                  className="relative flex items-center justify-center w-6 h-6"
                  whileHover={{ scale: 1.25 }}
                >
                  {/* Glowing halo pulse on active or hovered checkpoint */}
                  {(isActive || isHovered) && (
                    <motion.div
                      layoutId="activeHalo"
                      className="absolute inset-0 rounded-full bg-forest-400/20"
                  ...
                  <div
                    className={cn(
                      ...
                      isActive
                        ? "bg-forest-400 border-forest-200 scale-125 shadow-[0_0_12px_rgba(74,222,128,0.8)]"
                        : "bg-white/30 border-white/10 hover:bg-white/80"
                    )}
                  />
                </motion.div>
              </div>
